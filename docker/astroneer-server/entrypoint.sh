...
#
# FIX (vs. upstream barumel/docker-astroneer-server): the original here was
#   wait -n
#   [ "$?" -ge 0 ] || kill "$p1" "$p2"
#   wait
# Exit codes are never negative, so `-ge 0` is true for every possible exit
# status — the kill branch could never run. If the actual game server ($p1,
# run via Proton) died for any reason, the script fell straight through to a
# second bare `wait`, which then blocked forever on $p2 (the healthcheck
# helper, which doesn't exit on its own) — the container never exited, so
# Docker's restart policy never got a chance to recover it.
#
